...
# где каждый ключ должен быть уникальным и неизменяемым объектом, а каждое значение может быть любым объектом
vocabulary = {}
# создает словарь
my_dict = {'key1': 'value1', 'key2': 'value2', 'key3': 'value3'}
# key1 дает доступ к value1
dict(key1 = 'value1')
# более простая форма записи словаря, но работает только со строками
my_dict['key1']
# в коде пишем это что бы получить доступ к значению за которое отвечает key1
# словари работают с: числами, строками, кортежами и булевыми элементами (True/false), 
# но не работают с списками
# можно создавать словарь в словаре(смотри Lesson 8)
.get()
# аналог квадратных скобок
.clear()
# очищает словарь 
.pop(название ключа элемента)
# удаляет какойто элемент
.popitem()
# удаляет последний элеент словаря
.keys()
# получаем ключи словаря
.values()
# получаем значение ключей словаря
.items()
# получаем ключи и значения ключей словаря
.update()
update({ключ: значение}) 
# обновляем значение ключа
my_dict['key1'] = 'value4'
# аналог присваиваивания значение ключа
